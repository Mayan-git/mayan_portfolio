...
The personal portfolio of [**Mayan Prajapati**](https://github.com/Mayan-git). Built with Next.js 15, GSAP, and Lenis, it features scroll-driven animations, animated page transitions, smooth scrolling, and a working contact form.
...
```bash
git clone https://github.com/Mayan-git/Portfolio.git
cd Portfolio
npm install
```
...
The contact form sends mail via [Resend](https://resend.com), so `RESEND_API_KEY` must be an API key from your Resend account. Without a verified sending domain on Resend, the sandbox sender (`onboarding@resend.dev`) can only deliver to the email address associated with the Resend account itself — verify a domain in Resend to send to any recipient.
