...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!process.env.RESEND_API_KEY) {
      console.error('Email Error: RESEND_API_KEY env variable is not configured in this environment!');
      return NextResponse.json(
        {
          success: false,
          error: 'Email configuration error. RESEND_API_KEY is not set.',
        },
        {
          status: 500,
        },
    ...
    if (process.env.NODE_ENV === 'development') {
      try {
        ...
        return NextResponse.json({
          success: true,
          message: 'Message saved locally (email delivery failed, check messages.txt)!',
        });
      } catch (fsError) {
